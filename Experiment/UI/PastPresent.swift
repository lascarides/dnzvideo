...
class PastPresent: Section {
    
    override init() {
        
        super.init()
        
        section     = VideoSections.PastPresent
        voiceover   = SKAudioNode(url: URL(fileURLWithPath: "\(Settings.homePath)/voiceovers/02-past-present.m4a"))
        duration    = 8.081
